...
import { ModalContext } from '@/components/basic/PreviewSection/context'
import { numberFormatter, scaler } from '@/utils'

import Card from '@/components/basic/Card'
import Avatar from '@/components/basic/Avatar'
import IconButton from '@/components/basic/IconButton'

import thumbUpIcon from '@/assets/icons/icon-thumb-up-dark-gray.svg'
import thumbUpCircleIcon from '@/assets/icons/icon-thumb-up-circle-dark.svg'

import { SpaceCategories, SpaceSimple } from '@/interface/Space'

import tw from 'twin.macro'
import Icon from '@/components/basic/Icon'
import Device, { DeviceType } from '@/styles/Device'
```

위 코드를 보자. 실제로 프로젝트에 사용되고 있는 코드였다.
...
사용해본 결과 바로 거의 대부분의 파일들에서 린트에러를 나타내는 빨간 지렁이줄들이 생겼다.
그리고 린트 규칙에 맞게 자동 수정을 하면 내가 적용해놓은 모듈 타입의 순서에 따라 import 구문들을 정렬하는 결과를 볼 수 있다.
모듈 타입의 순서에 대해서는 좀 더 고민이 필요하다고 생각되지만 우선 이러한 기능에 대해서
프로젝트에 적용한 결과를 팀원들에게 공유하자 반응이 좋았다.
다음번 회의에서 팀원들과 논의를 통해 프로젝트의 코딩 컨벤션으로 정할 모듈 타입의 순서에 대해서 더 구체적으로 정해보기로 했다.
